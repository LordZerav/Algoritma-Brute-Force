...
def koordinat_titik(x1, y1, x2, y2):
    ...
    while x < x2: # Kondisi ketika nilai x < x2, maka selanjutnya
        x += 1 # Menambahkan nilai x sebesar 1 setiap iterasi
        y = round(y1 + m * (x - x1)) # Menghitung nilai y menggunakan rumus pers garis y = y1 + m(x-x1) di dalam fungsi round() untuk memperoleh hasil nilai y yang dibulatkan
        points.append((x, y)) # Menambahkan koordinat (x,y) ke dalam list points
        iterasi += 1 # Menambahkan iterasi
    
    # Loop While ini akan terus berjalan sampai nilai x mencapai x2
